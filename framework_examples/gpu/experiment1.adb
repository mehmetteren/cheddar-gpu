...
procedure experiment1 is
    ...
    (1 =>
      (Id => 1, kernel_count => 4, Stream => 1, period => 30, deadline => 30,
       kernels =>
        new Kernel_Array'
         (1 =>
           (id => 1, block_count => 2, block_size => 1_024, capacity => 10),
          2 =>
           (id => 2, block_count => 1, block_size => 1_024, capacity => 10),
          3 =>
           (id => 3, block_count => 8, block_size => 1_024, capacity => 10),
          4 =>
           (id => 4, block_count => 1, block_size => 1_024, capacity => 10))),
     2 =>
      (Id => 2, kernel_count => 3, Stream => 2, period => 40, deadline => 40,
       kernels =>
        new Kernel_Array'
         (1 =>
           (id => 1, block_count => 4, block_size => 1_024, capacity => 10),
          2 =>
           (id => 2, block_count => 2, block_size => 1_024, capacity => 10),
          3 =>
           (id => 3, block_count => 8, block_size => 1_024, capacity => 10))));
  ...
  TPC13_1 : constant TPC_ptr :=
   new TPC'
    (Id => 1, max_block_size => 1_024, resource_multiplier => 1, SMs => null);
  TPC13_2 : constant TPC_ptr :=
   new TPC'
    (Id => 2, max_block_size => 1_024, resource_multiplier => 3, SMs => null);

  TPC13ss : TPCList := (1 => TPC13_1, 2 => TPC13_2);

  stream_to_TPC13 : StreamTPCMap :=
   (1 => new TPCList'(1 => TPC13_1), 2 => new TPCList'(1 => TPC13_2));

  -- ------- ------- ------- -------

  TPC31_1 : constant TPC_ptr :=
   new TPC'
    (Id => 1, max_block_size => 1_024, resource_multiplier => 3, SMs => null);
  TPC31_2 : constant TPC_ptr :=
   new TPC'
    (Id => 2, max_block_size => 1_024, resource_multiplier => 1, SMs => null);

  TPC31ss : TPCList := (1 => TPC31_1, 2 => TPC31_2);

  stream_to_TPC31 : StreamTPCMap :=
   (1 => new TPCList'(1 => TPC31_1), 2 => new TPCList'(1 => TPC31_2));

  -- ------- ------- ------- -------

  TPC22_1 : constant TPC_ptr :=
   new TPC'
    (Id => 1, max_block_size => 1_024, resource_multiplier => 2, SMs => null);
  TPC22_2 : constant TPC_ptr :=
   new TPC'
    (Id => 2, max_block_size => 1_024, resource_multiplier => 2, SMs => null);

  TPC22ss : TPCList := (1 => TPC22_1, 2 => TPC22_2);

  stream_to_TPC22 : StreamTPCMap :=
   (1 => new TPCList'(1 => TPC22_1), 2 => new TPCList'(1 => TPC22_2));
