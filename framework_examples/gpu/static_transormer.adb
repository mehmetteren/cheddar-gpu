...
package body static_transfomer is
    ...
    begin
        ...
        display_system (System2 => generated_system);
        ...
        for i in 1 .. stream_to_TPC'Length loop

            for i in 1 .. tpcc.SMs'Length loop
                put_line
                   ("TPC " & tpcc.id'Img & " SM " &
                    to_string (tpcc.SMs (i).name));
            end loop;
        end loop;

        for dag of DAGs loop
            for tpc_index in 1 .. stream_to_tpc (dag.stream)'Length loop
        ...
        Write_To_Xml_File
           (transformed_system => transformed_system,
            File_Name          =>
               Suppress_Space
                  (To_Unbounded_String
