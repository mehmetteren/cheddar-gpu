...
package body gpu_generator is
    ...
    procedure gpu_generator
       (DAGs : in DAGList; stream_to_TPC : in out TPCList; TPCs : in out TPCList;
    ...
    begin
        ...
        for i in 1 .. TPC_count loop
            tpc_block_size := TPCs (i).block_size;
        ...
        for tpcc of stream_to_TPC loop
            for i in 1..tpcc.SMs'Length loop
            put_line ("TPC " & tpcc.id'Img & " SM " & to_string(tpcc.SMs(i).name));
            end loop;
        end loop;

        for dag of DAGs loop
            cur_tpc := stream_to_TPC (dag.stream);
            prev_tasks.Clear;
            cur_tasks.Clear;
            cur_task  := null;
            prev_task := null;

            for i in 1 .. dag.size loop -- for kernel in dag.kernels
                ...
                for j in 1 .. cur_kernel.block_count loop
                    ...
                    Add_Task
                       (My_Tasks => a_system.Tasks, A_Task => cur_task,
                        Name               =>
                           Suppress_Space
                              (To_Unbounded_String
                                  ("DAG" & Integer'Image (dag.id) & "-Kernel" &
                                   Integer'Image (i) & "-Block" &
                                   Integer'Image (j))),
                        -- TPC_x-SM_y
                        Cpu_Name           =>
                           Suppress_Space
                              ("TPC_" & Integer'Image(stream_to_TPC (dag.stream).id) & "-" &
                    ...
                    put_line ("Task " & to_string(cur_task.name) & " added to system");
                    ...
                    if i > 1 then
                        ...
                        for prev_taskk of prev_tasks loop
                               ...
                               (my_messages   => a_system.messages,
                                ...
                                name          =>
                                   Suppress_Space
                                      (To_Unbounded_String
                                          ("message:DAG" &
                                           Integer'Image (dag.id) & "-" &
                                           "node" & Integer'Image (i-1) &
                                           "_to_" & "node" &
                                           Integer'Image (i) & "Prev" & Integer'Image (prev_task_counter) & "-Block" & Integer'Image (j))),
                            ...
                            put_line ("Message " & to_string(message_ptr.name) & " added to system");
                        end loop;
                    end if;
